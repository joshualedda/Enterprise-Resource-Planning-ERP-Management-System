...
export default function Department({ statuses, filters: serverFilters = {} }) {
    ...
    const handleEdit = (statusRecord) => {
        ...
        setData({
            department_code: statusRecord.department_code || '',
            department_name: statusRecord.department_name || '',
    ...
    const submit = (e) => {
        ...
        if (editingStatus) {
            put(route('staff.hr.department.update', editingStatus.id), {
                onSuccess: () => {
                    closeModal();
            ...
            post(route('staff.hr.department.store'), {
                onSuccess: () => {
                    closeModal();
    ...
    const handleDelete = (id) => {
        if (confirm('Are you sure you want to delete this department?')) {
            router.delete(route('staff.hr.department.destroy', id), {
    ...
    return (
        <HRStaffLayout>
            <Head title="Department Setup" />
            <Toaster position="top-right" />

            <div className="space-y-6 animate-in fade-in duration-500">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Department</h1>
                        <p className="text-sm text-slate-400 font-medium mt-0.5">Manage company departments and divisions.</p>
                    </div>
                ...
                <Table
                    ...
                    subtitle="All configured company departments"
                    badgeCount={statuses.total}
                    badgeColor="bg-indigo-50 text-indigo-600"
                    columns={[
                        <button key="code" onClick={() => toggleSort('department_code')} className="flex items-center gap-1 hover:text-indigo-600 transition">
                            Code <SortIcon field="department_code" />
                        </button>,
                        <button key="name" onClick={() => toggleSort('department_name')} className="flex items-center gap-1 hover:text-indigo-600 transition">
                            Department Name <SortIcon field="department_name" />
                        </button>,
                        'Status',
                        'Actions'
                    ]}
                >
                    {sorted.length === 0 ? (
                        ...
                        <Tr key={statusItem.id}>
                            <Td>
                                <span className="text-sm font-bold text-slate-800">{statusItem.department_code}</span>
                            </Td>
                            <Td>
                                <span className="text-sm font-medium text-slate-600">{statusItem.department_name}</span>
                            </Td>
                            <Td>
                ...
                {statuses.last_page > 1 && (
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
                        <p className="text-xs text-slate-400 font-medium">
                            ...
                            <span className="font-black text-slate-600">{statuses.total}</span> departments
                        </p>
                        <Pagination
                            currentPage={statuses.current_page}
                            totalPages={statuses.last_page}
                            onPageChange={(page) =>
                                router.get(route('staff.hr.department'), {
                    ...
                    <div className="p-6">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-xl font-black text-slate-900">
                                {editingStatus ? 'Edit Department' : 'Add Department'}
                            </h2>
                        ...
                        <form onSubmit={submit} className="space-y-5">
                            <div>
                                ...
                                <TextInput
                                    ...
                                    required
                                />
                                <InputError message={errors.department_code} className="mt-1" />
                            </div>

                            <div>
                                <InputLabel value="Department Name" className={labelClass} />
                                <TextInput
                                    ...
                                    required
                                />
                                <InputError message={errors.department_name} className="mt-1" />
                            </div>
