...
export default function EmpStatus({ statuses, filters: serverFilters = {} }) {
    ...
    const handleEdit = (statusRecord) => {
        ...
        setData({
            employee_status_code: statusRecord.employee_status_code || '',
            employee_status_name: statusRecord.employee_status_name || '',
    ...
    const submit = (e) => {
        ...
        if (editingStatus) {
            put(route('staff.hr.emp-status.update', editingStatus.id), {
                onSuccess: () => {
                    closeModal();
            ...
            post(route('staff.hr.emp-status.store'), {
                onSuccess: () => {
                    closeModal();
    ...
    const handleDelete = (id) => {
        if (confirm('Are you sure you want to delete this employee status?')) {
            router.delete(route('staff.hr.emp-status.destroy', id), {
                onSuccess: () => toast.success('Employee Status deleted successfully!'),
            });
        }
    };
    ...
    return (
        <HRStaffLayout>
            <Head title="Employee Status Setup" />
            <Toaster position="top-right" />

            <div className="space-y-6 animate-in fade-in duration-500">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Employee Status</h1>
                        <p className="text-sm text-slate-400 font-medium mt-0.5">Manage types of employment status.</p>
                    </div>
                ...
                <Table
                    ...
                    subtitle="All configured employment statuses"
                    badgeCount={statuses.total}
                    badgeColor="bg-indigo-50 text-indigo-600"
                    columns={[
                        <button key="code" onClick={() => toggleSort('employee_status_code')} className="flex items-center gap-1 hover:text-indigo-600 transition">
                            Code <SortIcon field="employee_status_code" />
                        </button>,
                        <button key="name" onClick={() => toggleSort('employee_status_name')} className="flex items-center gap-1 hover:text-indigo-600 transition">
                            Status Name <SortIcon field="employee_status_name" />
                        </button>,
                        'Status',
                        'Actions'
                    ]}
                    ...
                    {sorted.length === 0 ? (
                        <tr>
                            <td colSpan={4} className="text-center py-12 text-slate-400 text-sm font-medium">
                                No employee statuses found matching your filters.
                            </td>
                        </tr>
                    ) : sorted.map((statusItem) => (
                        <Tr key={statusItem.id}>
                            <Td>
                                <span className="text-sm font-bold text-slate-800">{statusItem.employee_status_code}</span>
                            </Td>
                            <Td>
                                <span className="text-sm font-medium text-slate-600">{statusItem.employee_status_name}</span>
                            </Td>
                            <Td>
                ...
                {statuses.last_page > 1 && (
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
                        ...
                        <Pagination
                            ...
                            onPageChange={(page) =>
                                router.get(route('staff.hr.emp-status'), {
                    ...
                    <div className="p-6">
                        ...
                        <form onSubmit={submit} className="space-y-5">
                            <div>
                                ...
                                <TextInput
                                    value={data.employee_status_code}
                                    onChange={e => setData('employee_status_code', e.target.value)}
                                    className={inputClass}
                                    placeholder="e.g. REG"
                                    required
                                />
                                <InputError message={errors.employee_status_code} className="mt-1" />
                            </div>

                            <div>
                                <InputLabel value="Status Name" className={labelClass} />
                                <TextInput
                                    value={data.employee_status_name}
                                    onChange={e => setData('employee_status_name', e.target.value)}
                                    className={inputClass}
                                    placeholder="e.g. Regular"
                                    required
                                />
                                <InputError message={errors.employee_status_name} className="mt-1" />
                            </div>
                            ...
                            <div className="pt-4 flex gap-3">
                                ...
                                <PrimaryButton
                                    ...
                                    disabled={processing}
                                >
                                    {processing ? 'Saving...' : 'Save Employee Status'}
                                </PrimaryButton>
                            </div>
                        </form>
                    </div>
