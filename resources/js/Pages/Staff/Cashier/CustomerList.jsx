...
export default function CustomerList({ customers, filters = {}, flash = {} }) {
    ...
    useEffect(() => {
        ...
        const timeoutId = setTimeout(() => {
            router.get(route('staff.cashier.customer-list'), {
    ...
    return (
        <CashierStaffLayout>
            <Head title="Customer Management — Cashier" />
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 text-center">
                    <div className="relative max-w-xl mx-auto">
                        ...
                        <input
                            ...
                            className="w-full pl-9 pr-4 py-2.5 text-sm font-medium text-slate-700 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 outline-none transition placeholder-slate-400"
                        />
                    </div>
                </div>
                    ...
                    footer={
                        <div className="px-6 py-4 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-between gap-4 rounded-b-2xl">
                            ...
                            <Pagination
                                ...
                                onPageChange={(page) => {
                                    router.get(route('staff.cashier.customer-list'), { search, page }, { preserveState: true, preserveScroll: true });
                                }}
                            />
                        </div>
                    }
                >
                    {customers.data.map((user) => (
                        <Tr key={user.id}>
                            ...
                            <Td>
                                <button className="text-[10px] font-black text-emerald-600 bg-emerald-50 hover:bg-emerald-100 px-2.5 py-1 rounded-xl transition">
                                    View Activity
                                </button>
                            </Td>
                        </Tr>
