...
export default function Positions({ statuses, filters: serverFilters = {} }) {
    ...
    const handleEdit = (statusRecord) => {
        ...
        setData({
            position_code: statusRecord.position_code || '',
            position_name: statusRecord.position_name || '',
    ...
    const submit = (e) => {
        ...
        if (editingStatus) {
            put(route('staff.hr.position.update', editingStatus.id), {
                onSuccess: () => {
                    closeModal();
            ...
            post(route('staff.hr.position.store'), {
                onSuccess: () => {
                    closeModal();
    ...
    const handleDelete = (id) => {
        if (confirm('Are you sure you want to delete this position?')) {
            router.delete(route('staff.hr.position.destroy', id), {
    ...
    return (
        <HRStaffLayout>
            <Head title="Position Setup" />
            <Toaster position="top-right" />

            <div className="space-y-6 animate-in fade-in duration-500">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Positions</h1>
                        <p className="text-sm text-slate-400 font-medium mt-0.5">Manage company positions and job titles.</p>
                    </div>
                ...
                <Table
                    title="📋 Position List"
                    subtitle="All configured company job positions"
                    badgeCount={statuses.total}
                    badgeColor="bg-indigo-50 text-indigo-600"
                    columns={[
                        <button key="code" onClick={() => toggleSort('position_code')} className="flex items-center gap-1 hover:text-indigo-600 transition">
                            Code <SortIcon field="position_code" />
                        </button>,
                        <button key="name" onClick={() => toggleSort('position_name')} className="flex items-center gap-1 hover:text-indigo-600 transition">
                            Position Name <SortIcon field="position_name" />
                        </button>,
                        'Status',
                        'Actions'
                    ]}
                >
                    {sorted.length === 0 ? (
                        ...
                        <Tr key={statusItem.id}>
                            <Td>
                                <span className="text-sm font-bold text-slate-800">{statusItem.position_code}</span>
                            </Td>
                            <Td>
                                <span className="text-sm font-medium text-slate-600">{statusItem.position_name}</span>
                            </Td>
                            <Td>
                ...
                {statuses.last_page > 1 && (
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
                        <p className="text-xs text-slate-400 font-medium">
                            ...
                            <span className="font-black text-slate-600">{statuses.total}</span> positions
                        </p>
                        <Pagination
                            currentPage={statuses.current_page}
                            totalPages={statuses.last_page}
                            onPageChange={(page) =>
                                router.get(route('staff.hr.position'), {
                    ...
                    <div className="p-6">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-xl font-black text-slate-900">
                                {editingStatus ? 'Edit Position' : 'Add Position'}
                            </h2>
                        ...
                        <form onSubmit={submit} className="space-y-5">
                            <div>
                                ...
                                <TextInput
                                    ...
                                    required
                                />
                                <InputError message={errors.position_code} className="mt-1" />
                            </div>

                            <div>
                                <InputLabel value="Position Name" className={labelClass} />
                                <TextInput
                                    ...
                                    required
                                />
                                <InputError message={errors.position_name} className="mt-1" />
                            </div>
