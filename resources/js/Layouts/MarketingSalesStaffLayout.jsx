...
const roleConfig = {
    8: { label: 'Marketing & Sales', color: 'bg-rose-100 text-rose-700', dot: 'bg-rose-600' },
};
...
// ---------------------------------------------------------------------------
const sampleNotifs = [
    { id: 1, icon: '📊', title: 'Campaign launched', body: 'Summer Silk Sale campaign is now live.', time: '5m ago', unread: true },
    { id: 2, icon: '🎯', title: 'Promo code redeemed', body: 'SILK20 used by Maria Santos.', time: '15m ago', unread: true },
    { id: 3, icon: '📈', title: 'Sales milestone', body: 'Monthly target 80% achieved.', time: '1h ago', unread: false },
    { id: 4, icon: '👤', title: 'New customer signup', body: 'Ana Cruz joined via referral.', time: '3h ago', unread: false },
];
    ...
    return (
        <div className="relative" ref={ref}>
            <button onClick={() => setOpen(v => !v)}
                className="relative p-2 rounded-xl text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-all"
            >
            ...
            {open && (
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl shadow-slate-200/80 border border-slate-100 z-50 overflow-hidden">
                    <div className="px-4 py-3 border-b border-slate-50 flex items-center justify-between">
                        ...
                        {unread > 0 && (
                            <button onClick={markAll} className="text-[10px] font-bold text-rose-500 hover:text-rose-600 transition">
                                Mark all read
                            </button>
                        )}
                    </div>
                    <div className="max-h-72 overflow-y-auto divide-y divide-slate-50">
                        {notifs.map(n => (
                            <div key={n.id} className={`px-4 py-3 flex gap-3 items-start ${n.unread ? 'bg-rose-50/40' : ''}`}>
                                ...
                                {n.unread && <span className="w-2 h-2 rounded-full bg-rose-400 mt-2 flex-shrink-0" />}
                            </div>
                        ))}
                    </div>
                </div>
        ...
        <div className="relative" ref={ref}>
            ...
            >
                <div className="relative">
                    <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-rose-500 to-pink-600 flex items-center justify-center text-white font-black text-sm shadow-md shadow-rose-200 group-hover:shadow-rose-300 transition-shadow">
                        {initials}
                    </div>
            ...
            {open && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-2xl shadow-slate-200/80 border border-slate-100 z-50 overflow-hidden">
                    ...
                    <div className="p-1.5">
                        <Link href={route().has('staff.marketing-salesprofile') ? route('staff.marketing-salesprofile') : '#'}
    ...
    return (
        <div className="mb-1 block">
            <button
                ...
                className={`w-full flex items-center justify-between px-4 py-2 text-sm font-medium rounded-xl transition-all duration-200 group ${isActive ? 'text-rose-800 bg-rose-50' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
                    }`}
            >
...
// MAIN LAYOUT
// ---------------------------------------------------------------------------
export default function MarketingSalesStaffLayout({ children }) {
    const page = usePage();
    ...
    const navigation = useMemo(() => [
        ...
        {
            name: 'Profile',
            href: route().has('staff.marketing-salesprofile') ? route('staff.marketing-salesprofile') : '#',
        ...
        <div className="flex h-screen bg-slate-50 font-sans antialiased">
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-[268px] bg-white border-r border-slate-100 shadow-xl shadow-slate-200/60 flex flex-col transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 lg:shrink-0`}>
                ...
                <div className="px-5 pt-6 pb-5 flex-shrink-0">
                    <Link href="/" className="flex items-center gap-2.5">
                        ...
                        <div>
                            ...
                            <p className="text-[9px] font-bold text-rose-500 uppercase tracking-widest mt-0.5">Marketing & Sales</p>
                        </div>
                    </Link>
                </div>
                ...
                <div className="px-4 pb-5 flex-shrink-0">
                    <div className="relative overflow-hidden bg-gradient-to-br from-rose-500 to-pink-600 rounded-2xl p-4 text-white">
                        ...
                        <div className="relative mt-3 pt-3 border-t border-white/15 flex items-center justify-between">
                            <span className="text-[10px] font-bold text-rose-100">Marketing Portal</span>
                        </div>
                    </div>
                </div>
            </aside>
